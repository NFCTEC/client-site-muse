...
export const Route = createFileRoute("/contact")({
  head: () => ({
    meta: [
      { title: "联系我们 — NFCTec" },
      { name: "description", content: "联系 NFCTec 方案工程师，1 个工作日内获取专属报价与样品。" },
      { property: "og:title", content: "Contact — NFCTec" },
      { property: "og:description", content: "Talk to our solution engineers." },
    ],
  }),
  component: Contact,
});
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-10">
        ...
        <div className="mt-16 grid lg:grid-cols-5 gap-8 lg:gap-12">
          ...
          <form
            ...
            className="lg:col-span-3 rounded-3xl border border-border bg-card p-8 lg:p-12"
          >
            {done ? (
              ...
              <>
                ...
                <div className="mt-5">
                  ...
                  <textarea
                    ...
                    rows={5}
                    required
                    className="w-full rounded-xl border border-border bg-background px-4 py-3 text-sm outline-none focus:border-foreground transition-colors resize-none"
                  />
                </div>
                <button
                  type="submit"
                  className="mt-8 inline-flex items-center gap-2 rounded-full bg-foreground text-background px-7 py-3.5 text-sm font-medium hover:bg-primary transition-colors"
                >
                  {tr("form.submit")}
                  <ArrowRight size={16} />
                </button>
          ...
          <aside className="lg:col-span-2 space-y-4">
            {[
              ...
              { icon: MapPin, label: tr("contact.addr"), value: tr("footer.addr") },
            ].map((c) => {
              const Icon = c.icon;
              return (
                <div key={c.label} className="rounded-2xl border border-border bg-surface p-7">
  ...
  return (
    <div>
      ...
      <input
        ...
        name={name}
        required={required}
        className="w-full rounded-xl border border-border bg-background px-4 py-3 text-sm outline-none focus:border-foreground transition-colors"
      />
    </div>
  );
}
