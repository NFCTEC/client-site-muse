...
function normalizeModuleConfig(raw: unknown): ModuleDisplayConfig {
  if (!raw || typeof raw !== "object") return { ...DEFAULT_MODULE };
...
export function isModuleEnabled(config: SiteDisplayConfig, key: ModuleKey): boolean {
  return config.modules[key]?.enabled !== false;
}

export function isModuleInNav(config: SiteDisplayConfig, key: ModuleKey): boolean {
  const mod = config.modules[key];
  return mod?.enabled !== false && mod?.showInNav !== false;
}

export type LocaleRouteLoaderData = {
  displayConfig: SiteDisplayConfig;
  navSolutions: { id: string; slug: string; name: string }[];
  navProducts: { id: string; slug: string; name: string; category: string }[];
};

export function getLocaleLoaderData(matches: { routeId: string; loaderData?: unknown }[]): LocaleRouteLoaderData | null {
  const row = matches.find((m) => m.routeId === "/$locale");
  if (!row?.loaderData) return null;
  return row.loaderData as LocaleRouteLoaderData;
}
